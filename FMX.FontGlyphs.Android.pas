...
begin
  ...
  try
    ...
    FontName := TPath.Combine(TPath.GetDocumentsPath, CurrentSettings.Family + '.TTF');
    if FileExists(FontName) then begin
        TypeFace := TJTypeface.JavaClass.createFromFile(StringToJString(FontName));
    end else begin
        Typeface := TJTypeface.JavaClass.create(FamilyName, TypefaceFlag);
    end;
